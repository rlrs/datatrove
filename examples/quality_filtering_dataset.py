...
def main():
    for dataset, limit in zip(INPUT_DATASETS, LIMITS):
        ...
        pipeline = [
            JsonlReader(data_folder=folder, limit=limit, doc_progress=True, default_metadata={"url": "Unknown"}),
            URLFilter(exclusion_writer=JsonlWriter(f"{FILTER_OUTPUT_PATH}/{dataset}/1_url_filter/", output_filename="${rank}.jsonl.gz")),
            LanguageFilter(
            ...
            GopherRepetitionFilter(
                exclusion_writer=JsonlWriter(f"{FILTER_OUTPUT_PATH}/{dataset}/3_repetition_filter/", output_filename="${rank}.jsonl.gz"),
                language=Languages.danish
            ),
            # GopherQualityFilter(
            #     exclusion_writer=JsonlWriter(f"{FILTER_OUTPUT_PATH}/{dataset}/4_gopher_quality_filter/", output_filename="${rank}.jsonl.gz"),
            #     language=Languages.danish
            # ),
            # FineWebQualityFilter(
            ...
            #     language=Languages.danish
            # ),
            # ParquetWriter(f"{FILTERING_OUTPUT_PATH}/output/"), # parquetwriter is broken...
